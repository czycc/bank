...
//        });
        ...
        $grid->column('out_task', '拓客任务')->display(function () {
            return  OutTaskUser::where('user_id', $this->id)->where('created_at', '>', Carbon::today())->count();
        });
        $grid->column('visit_task', '来访任务')->display(function () {
            return  VisitTaskUser::where('user_id', $this->id)->where('created_at', '>', Carbon::today())->count();
        });
        $grid->column('new_task', '老带新任务')->display(function () {
            return  NewTaskUser::where('user_id', $this->id)->where('created_at', '>', Carbon::today())->count();
        });
        $grid->column('invite_task', '邀约任务')->display(function () {
            return  InviteTaskUser::where('user_id', $this->id)->where('created_at', '>', Carbon::today())->count();
        });
        return $grid;
    }
